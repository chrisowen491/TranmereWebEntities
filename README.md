...
The SAM CLI installs dependencies defined in `Entities/package.json`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
$ sam local invoke Entities --event events/event.json
```
...
```bash
$ npm run build
$ sam deploy --stack-name Entities --capabilities CAPABILITY_IAM --s3-bucket tranmere-web-api
```
...
```bash
$ sam logs -n EntitiesFunction --stack-name Entities --tail
```
...
```bash
$ aws cloudformation delete-stack --stack-name Entities
```
